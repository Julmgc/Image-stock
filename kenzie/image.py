...
def create_files_directory_variable():
  if not os.path.exists("./images-used"):
    os.mkdir("./images-used")
...
def upload_files_type():
  try:
    ...
    if png_file:
      ...
      if os.path.exists('./images-used/PNG'):
      ...
      if os.path.isfile(f'./images-used/JPG/{filename_variable}'):
        return "File already exists", 409
      if os.path.exists('./images-used/JPG'):
      ...
      if os.path.isfile(f'./images-used/GIF/{filename_variable}'):
        return "File already exists", 409
      if os.path.exists('./images-used/GIF'):
...
def get_file_by_format(type):
  try:
    ...
    if incoming_tipo == 'png' and os.path.exists("./images-used/PNG"):
      only_png_files = [f for f in listdir('./images-used/PNG') if isfile(join('./images-used/PNG', f))]
      return jsonify(only_png_files)
 
    elif incoming_tipo == 'jpg' and os.path.exists("./images-used/JPG"):
      only_jpg_files = [f for f in listdir('./images-used/JPG') if isfile(join('./images-used/JPG', f))]
      return jsonify(only_jpg_files)
    
    elif incoming_tipo == 'gif' and os.path.exists("./images-used/GIF"):
      only_gif_files = [f for f in listdir('./images-used/GIF') if isfile(join('./images-used/GIF', f))]
...
def download_specific_file(file_name):
  try:
    ...
    if png_file:
      return send_from_directory(directory='../images-used/PNG', path=f"{file_name}", as_attachment=True)
    elif jpg_file:
      return send_from_directory(directory='../images-used/JPG', path=f"{file_name}", as_attachment=True)
    else:
      return send_from_directory(directory='../images-used/GIF', path=f"{file_name}", as_attachment=True)
...
def donwload_zip_format_files():
  try:
    ...
    os.system(f'zip -{rate_compression} -r {file_name.name}.zip ./images-used/{file_type_uppercase}')
